        ...
        <div id="menu-mobile" class="hidden md:hidden mt-4 bg-slate-900/90 rounded-xl border border-slate-700/50 p-4 space-y-2">
            @if(Auth::check())
                ...
                @elseif(Auth::user()->role === 'student')
                    ...
                    <a href="{{ route('attendance.history') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg text-white hover:bg-blue-600/30 transition-colors duration-150">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <span class="font-semibold text-sm">Riwayat Kehadiran</span>
                    </a>
