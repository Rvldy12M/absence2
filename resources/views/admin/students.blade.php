...
.std-inner { max-width: 1100px; margin: 0 auto; }
...
.std-actions { display: flex; gap: 0.6rem; align-items: flex-start; margin-top: 6px; flex-wrap: wrap; }
...
#studentsTable_wrapper .dataTables_paginate .paginate_button {
...
#studentsTable_wrapper .dataTables_paginate .paginate_button:hover {
    background: var(--accent-light) !important;
    color: var(--accent) !important;
    border-color: var(--accent) !important;
}
#studentsTable_wrapper .dataTables_paginate .paginate_button.current,
#studentsTable_wrapper .dataTables_paginate .paginate_button.current:hover {
    background: var(--accent) !important;
    color: #fff !important;
    border-color: var(--accent) !important;
}
#studentsTable_wrapper .dataTables_paginate .paginate_button.disabled,
#studentsTable_wrapper .dataTables_paginate .paginate_button.disabled:hover {
    opacity: 0.35;
    cursor: not-allowed;
    background: var(--surface) !important;
...
table#studentsTable thead tr {
    background: var(--surface-2);
    border-bottom: 2px solid var(--border-strong);
}
table#studentsTable thead th {
...
table#studentsTable thead th.sorting_asc::after,
table#studentsTable thead th.sorting_desc::after { color: var(--accent) !important; }

table#studentsTable tbody tr { transition: background 0.1s; }
table#studentsTable tbody tr:hover { background: #f4f6fd !important; }
table#studentsTable tbody tr:nth-child(odd)  { background: var(--surface); }
table#studentsTable tbody tr:nth-child(even) { background: var(--surface-2); }
table#studentsTable tbody td {
    padding: 0.85rem 1rem !important;
...
table#studentsTable tbody tr:last-child td { border-bottom: none !important; }

#studentsTable_processing {
...
@media (max-width: 640px) {
    .std-title { font-size: 1.5rem; }
    table#studentsTable thead th,
    table#studentsTable tbody td { padding: 0.7rem 0.7rem !important; }
}
</style>
...
<div class="std-inner">
    ...
    <div class="std-header">
        <div>
            <span class="std-eyebrow">🎓 Data Siswa</span>
            <h1 class="std-title">List Siswa</h1>
        </div>
        <div class="std-actions">
            <a href="{{ route('admin.students.export') }}" class="btn-export">
    ...
    {{-- Filter + Table Card --}}
    <div class="std-card p-4 border-b border-slate-200">
        <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
            <div>
                <label for="classFilter" class="block text-xs font-semibold text-slate-600">Filter Kelas</label>
                <select id="classFilter" class="mt-1 rounded-lg border border-slate-300 px-3 py-2 text-sm w-full md:w-56">
            ...
            <div>
                <label for="searchInput" class="block text-xs font-semibold text-slate-600">Cari</label>
                <input id="searchInput" type="text" placeholder="Cari nama atau email" class="mt-1 rounded-lg border border-slate-300 px-3 py-2 text-sm w-full md:w-72" />
            </div>
        </div>
    </div>
    <div class="std-card">
...
$(document).ready(function () {
    const studentsTable = $('#studentsTable').DataTable({
        ...
        columns: [
            { data: 'id', name: 'id' },
            { data: 'name', name: 'name' },
            { data: "class_name", title: "Class", orderable: false },
